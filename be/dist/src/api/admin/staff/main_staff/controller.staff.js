...
const createStaff = async (request, response, next) => {
    ...
    try {
        ...
        let check_if_user_exist = await model_auth_1.USER.findOne({ email: staff_body.email });
        ...
        if (check_if_user_exist) {
            ...
            check_if_user_exist = await model_auth_1.USER.findByIdAndUpdate(check_if_user_exist.id, {
...
const updateStaff = async (request, response, next) => {
    try {
        ...
        else if (user?.permissions.includes(general_factory_1.PermissionsE.EDIT_STAFF)) {
...
const getMyStaffProfile = async (request, response, next) => {
    const findStaff = await model_staff_1.STAFF.findOne({ user: request.user.id }, "first_name, last_name, address, bank_details, username branch").populate("BRANCH");
    if (!findStaff)
