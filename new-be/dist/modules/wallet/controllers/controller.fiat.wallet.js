...
*/
export const createNewFiatWallet = catchAsync(async (req, res) => {
    const wallet = await walletService.createFiatWallet(Object.assign(Object.assign({}, req.body), { userId: req.user.id }));
...
//   res.send(wallets);
// });
export const getAllUserFiatWallets = catchAsync(async (req, res) => {
    const filter = Object.assign(Object.assign({}, pick(req.query, ['currencyCode', 'isActive'])), { userId: req.user.id });
    const options = pick(req.query, ['sort', 'limit', 'page']);
    const result = await walletService.queryCurrencyFiatWallets(filter, options);
    res.send(result);
});
export const deactivateUserFiatWallet = catchAsync(async (req, res) => {
    if (typeof req.params['id'] === 'string') {
        await walletService.deactivateCurrencyWalletById(new mongoose.Types.ObjectId(req.user.id), new mongoose.Types.ObjectId(req.params['id']));
 ...
 */
export const createNewDepositRequest = catchAsync(async (req, res) => {
    const depositRequest = await walletService.createDepositRequest(Object.assign(Object.assign({}, req.body), { userId: req.user.id }));
    res.status(httpStatus.CREATED).send(depositRequest);
});
export const queryCurrencyFiatWallets = catchAsync(async (req, res) => {
    const filter = Object.assign({}, pick(req.query, ['currencyCode', 'isActive']));
    const options = pick(req.query, ['sort', 'limit', 'page']);
    const wallets = await walletService.queryCurrencyFiatWallets(filter, options);
    res.send(wallets);
});
export const getFiatWalletsByUserId = catchAsync(async (req, res) => {
    const userId = new mongoose.Types.ObjectId(req.user.id);
    const result = await walletService.getFiatWalletsByUserId(userId);
    res.send(result);
});
export const getFiatWalletByUserIdAndCurrencyCode = catchAsync(async (req, res) => {
    const userId = req.user.id;
    const { currencyCode } = req.params;
    if (!currencyCode)
        throw new ApiError(httpStatus.BAD_REQUEST, 'currencyCode is required');
    const result = await walletService.getFiatWalletByUserIdAndCurrencyCode(userId, currencyCode);
    res.send(result);
});
export const getExchangeFiatWallet = catchAsync(async (req, res) => {
    const { currencyCode } = req.params;
    if (!currencyCode)
        throw new ApiError(httpStatus.BAD_REQUEST, 'currencyCode is required');
...
//   res.send(result);
// });
//# sourceMappingURL=controller.fiat.wallet.js.map