...
import { FXS } from '../../admin/exchangeWallets';
import ApiError from '../../errors/ApiError';
import { getExchangeRateInUSDT, getFirstExchange } from '../../exchange/services/service.engine.exchange';
...
import { GeneratePrefixType, GeneratekeyE } from '../../utils/referenceGenerator';
...
export async function getMainAccount() {
    ...
    const data = response.data.filter((x) => x.isDeleted === false &&
        x.isArchived === false &&
...
export async function createCryptoSubAccount(userId) {
    ...
    if (!response.data)
        throw new ApiError(httpStatus.NOT_FOUND, 'Sub account not created');
    // create sub Account
 ...
 */
export async function getSubAccount(userId) {
    const subAccount = await CRYPTO_SUB_ACCOUNT.findOne({ userId });
    if (!subAccount)
        throw new ApiError(httpStatus.NOT_FOUND, 'Sub account not found');
    return subAccount;
}
/**
 ...
 * @return the subAccount object.
 */
export async function getSubAccountByUid(uid) {
 ...
 */
export const createCryptoWallet = async (walletPayload, isExchange = false) => {
    var _a;
    ...
    const getImage = currencyImage === null || currencyImage === void 0 ? void 0 : currencyImage.find((e) => e.symbol && e.symbol.toUpperCase() === walletPayload.currencyCode);
    const walletBody = {
 ...
 */
export const queryCryptoWallets = async (filter, options) => {
    ...
    //   })
    // );
    return Object.assign(Object.assign({}, wallets), { results: rsult });
};
/**
 * Get wallet by id
 ...
 */
export const getCryptoWalletById = async (id) => {
 ...
 * @returns {Promise<ICryptoHDWalletDoc | null>}
 */
export const getCryptoWalletByIdAndCurrencyCode = async (id, currencyCode) => {
...
export async function getAllUsersCryptoAccounts(userId) {
...
export async function syncCryptoWalletWithBitPwr(id) {
    const wallet = await CRYPTO_HD_WALLETS.findById(id);
    if (!wallet)
        throw new ApiError(httpStatus.NOT_FOUND, 'Wallet not found');
 ...
 * @returns {Promise<ICryptoHDWalletDoc | null>}
 */
export const getCryptoWalletsByUserId = async (userId) => queryCryptoWallets({ userId }, {});
export const getCryptoWalletCumulativeBalanceByUserId = async (userId) => {
 ...
 * @returns {Promise<ICryptoHDWalletDoc | null>}
 */
export const getCryptoWalletByUserIdAndCurrencyCode = async (userId, currencyCode) => {
 ...
 */
export const getExchangeCryptoWallet = async (currencyCode) => {
    var _a;
    const getAssets = await getAllTradableAssets();
    const getAsset = getAssets.find((asset) => asset.assetType === currencyCode &&
    ...
    const address = addresses.data.find((ad) => config.env === 'development'
        ? ad.mode === 'TEST' &&
            ad.network === 'TESTNET' &&
    ...
    // create a deposit address for the user
    const currencyImage = await walletCurrencyService.tradableAssetsPlusIcons();
    const getImage = currencyImage === null || currencyImage === void 0 ? void 0 : currencyImage.find((e) => e.symbol && e.symbol.toUpperCase() === currencyCode);
    if (!findInDb) {
        const walletBody = {
 ...
 */
export const updateCryptoWalletById = async (walletId, currencyCode, updateBody) => {
 ...
 * @return a Promise that resolves to either an ICryptoHDWalletDoc object or null.
 */
export const deleteCryptoWalletById = async (id) => {
 ...
 */
export const withdrawCrypto = async (transactionPayload) => {
    const coinCode = transactionPayload.currencyCode;
    ...
    await simpleSendCryptoAPI({
        ...
        fromAddress: getWallet.address,
        toAmount: feeAmount,
        description: `transfer from user to admin withdrawal fee${generateAlphanumericReference(15, GeneratekeyE.alphanum, GeneratePrefixType.TRANSFER)}`,
        subAccountId: getSubAcct.uid,
    });
    if (!cryptoTx) {
 ...
 */
export const exchangeCrypto = async (userWallet, currencyCode, amount, mode) => {
    try {
        const coinCode = currencyCode;
...
export async function getWalletByWalletAddress(address) {
...
//# sourceMappingURL=service.crypto.wallet.js.map