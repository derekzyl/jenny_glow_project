...
class CrudS {
     ...
     */
    static async create(MyModel, data, finder) {
     ...
     * created'
     */
    static async createMany(MyModel, data, finder) {
        if (finder.length > 0)
            await Promise.all(finder.map(async (findr) => {
     ...
     */
    static async update(MyModel, data, filter) {
        const dataF = [];
        if (Array.isArray(MyModel)) {
            await Promise.all(MyModel.map(async (model) => {
     ...
     */
    static async getMany(MyModels, query, populate, category = null) {
        const all = [];
     ...
     */
    static async delete(MyModel, data) {
        if (Array.isArray(MyModel)) {
            Promise.all(MyModel.map(async (model) => {
     ...
     * - data: an array containing the fetched data
     */
    static async getOne(MyModel, data, populate) {
