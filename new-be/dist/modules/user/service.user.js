...
import { TOKENS, tokenTypes } from '../token';
import { verifyOtpToken } from '../token/service.token';
 ...
 * @returns {Promise<IUserDoc | null>}
 */
export const changeUserPasswordById = async (userId, updatePasswordBody) => {
    const user = await getUserById(userId);
    if (!user) {
        throw new ApiError(httpStatus.NOT_FOUND, 'User not found');
    }
 ...
 */
export const getUserPinByUserId = async (userId) => USER_PINS.findOne({ userId });
/**
 * Create a user pin
 * @param {NewCreatedUserPin} userPinBody
 * @returns {Promise<IUserPinDoc>}
 */
export const createUserPin = async (userPinBody) => {
...
export async function checkIfUserHasCreatedPin(userId) {
 ...
 * @returns {Promise<IUserPinDoc | null>}
 */
export const updateUserPinByUserId = async (userId, updateBody) => {
 ...
 * @returns {Promise<IUserPinDoc | null>}
 */
export const deleteUserPinByUserId = async (userId) => {
    const userPin = await getUserPinByUserId(userId);
    if (!userPin) {
        throw new ApiError(httpStatus.NOT_FOUND, 'User pin not found');
    }
 ...
 * @returns {Promise<IUserDoc>}
 */
export const verifyUserWithPin = async (id, pin) => {
    logger.info(`pin ${pin}`);
