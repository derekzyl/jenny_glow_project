...
const insertUsers = async (users) => {
    await User.insertMany(users.map((user) => (Object.assign(Object.assign({}, user), { password: hashedPassword }))));
};
describe('User routes', () => {
    ...
    describe('GET /api/v1/users', () => {
        ...
        test('should correctly apply filter on the name field(s)', async () => {
            await insertUsers([userOne, userTwo, admin]);
            const res = await request(app)
                .get('/api/v1/users')
                .set('Authorization', `Bearer ${adminAccessToken}`)
                .query({ firstName: userOne.firstName, lastName: userOne.lastName })
                .send()
                .expect(httpStatus.OK);
            expect(res.body).toEqual({
                results: expect.any(Array),
        ...
        test('should correctly sort the returned array if multiple sorting criteria are specified', async () => {
            ...
            const res = await request(app)
                .get('/api/v1/users')
                .set('Authorization', `Bearer ${adminAccessToken}`)
                .query({ sort: 'role:desc,firstName:asc' })
                .send()
                .expect(httpStatus.OK);
            expect(res.body).toEqual({
                results: expect.any(Array),
    ...
    describe('PATCH /api/v1/users/:userId', () => {
        ...
        test('should return 200 and successfully update user if admin is updating another user', async () => {
            await insertUsers([userOne, admin]);
            const updateBody = { firstName: faker.name.firstName(), lastName: faker.name.lastName() };
            await request(app)
                .patch(`/api/v1/users/${userOne._id}`)
                .set('Authorization', `Bearer ${adminAccessToken}`)
                .send(updateBody)
