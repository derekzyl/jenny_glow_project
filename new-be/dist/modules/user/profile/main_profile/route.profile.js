...
profileRouter
    .route('/')
    .post(auth(allPermissions.User.Manage, allPermissions.User.Create), MULTER_UPLOAD.fields([{ name: 'inventoryReceipt', maxCount: 1 }]), formFileHandler([{ name: 'inventoryReceipt' }]), ProfileIndex.create_profile)
    .get(auth(allPermissions.User.Manage, allPermissions.User.GetAll), ProfileIndex.get_all_profile);
...
profileRouter
    .route("/user")
    .get(auth(), ProfileIndex.get_profile_by_user_id)
    .patch(auth(), MULTER_UPLOAD.fields([{ name: 'inventoryReceipt', maxCount: 1 }]), formFileHandler([{ name: 'inventoryReceipt' }]), ProfileIndex.update_profile_by_user_id)
    .delete(auth(), ProfileIndex.delete_profile_by_user_id);
