...
reviewRouter
    .route('/')
    .post(auth(), // Assuming the user needs to be authenticated to create a review
...
reviewRouter
    ...
    .patch(auth(allPermissions.Review.Update, allPermissions.Review.Manage), // Assuming the user needs to be authenticated to update a review
validate(reviewValidation.updateReview), reviewController.updateReview)
    .delete(auth(allPermissions.Review.Delete, allPermissions.Review.Manage), // Assuming only authorized users can delete reviews
...
reviewRouter.route('/user/pending').get(auth(), // Assuming the user needs to be authenticated to view their pending reviews
validate(reviewValidation.getPendingReviewByUser), reviewController.getPendingReviewByUser);
reviewRouter.route('/user/review').patch(auth(), // Assuming the user needs to be authenticated to update their review
validate(reviewValidation.updateUserReview), reviewController.updateUserReview);
export { reviewRouter };
