...
export default class Base {
  ...
  public async request(path: string, payload: any, method?: Method, callback?: Function): Promise<AxiosResponse> {
    ...
    if (requestMethod === 'GET') {
      ...
      const data = removeProp(payload, ['excludeQuery', 'method']);
      requestOptions[dataKey] = RaveUtils.initDefaultValue({ ...data }, {});
    }
  ...
  public encrypt(data: any): string {
    const encryptionKey: string = Security.getEncryptionKey(this.getSecretKey());
  ...
  private static handleAxiosError(error: any, callback?: Function) {
    if (axios.isAxiosError(error)) {
      if (error.response) {
 ...
 */
export function getClassInstance(publicKey: string, secretKey: string, baseUrl?: string): Base {
