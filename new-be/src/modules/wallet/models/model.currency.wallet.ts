...
import { ICurrencyDoc, ICurrencyModel } from '../interfaces/interfaces.currency.wallet';

const currencySchema = new mongoose.Schema<ICurrencyDoc, ICurrencyModel>(
  {
    name: {
      type: String,
      uppercase: true,
 ...
 */
currencySchema.static(
  'isCodeTaken',
  async function (code: string, excludeCurrencyId: mongoose.Types.ObjectId): Promise<boolean> {
    const currency = await this.findOne({ code, _id: { $ne: excludeCurrencyId } });
    return !!currency;
  }
);

const CURRENCIES = mongoose.model<ICurrencyDoc, ICurrencyModel>('CURRENCIES', currencySchema);
