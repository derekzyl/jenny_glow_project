...
import { FileDataI, FileValueI } from './file_handler.interface';
 ...
 * string.
 */
export async function stringFileHandler(
  file: string,
  folderName: string,
  key: string,
  useEmailAndKey: boolean,
  user?: IUserDoc
) {
  ...
  if (useEmailAndKey && user) d = `${user.email.split('.')[0]}/${key as string}/`;
...
function keyz(field: { name: string }[]): string[] {
...
export const formFileHandler =
  (field: { name: string }[], folderName: string = 'paypaddy', useEmailAndKey: boolean = false) =>
      ...
      try {
        ...
        await Promise.all(
          allKeys.map(async (key) => {
            body[key] = '';
            const d = request.user && request.user.email ? `${request.user.email.split('@')[0]}/${key as string}/` : '';
            if (fieldValues.includes(key)) {
              logger.info(`${key} is a file `);
              if (Array.isArray(request.files)) {
                await Promise.all(
                  request.files.map(async (file) => {
                    const fileDat: FileValueI = {
                      fileData: {
                        buffer: file.buffer,
                ...
                await Promise.all(
                  request.files[key]!.map(async (file) => {
                    const fileDat: FileValueI = {
                      fileData: {
                        buffer: file.buffer,
                        encoding: file.encoding,
